            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title', $post->title) }}" maxlength="80" class="w-full border rounded p-2" required>
                    ...
                    <div>
                        ...
                        <select name="type" class="w-full border rounded p-2" required>
                            <option value="tip" @selected(old('type', $post->type) === 'tip')>Tip</option>
                            <option value="news" @selected(old('type', $post->type) === 'news')>Novedad</option>
                        </select>
                    ...
                    @if($post->image)
                        ...
                        <div>
                            ...
                            <img src="{{ $postImageUrl }}" alt="Imagen publicación" class="w-40 h-40 object-cover rounded border">
                        </div>
                    @endif

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block mb-1 font-medium">Fecha inicio (solo Novedad)</label>
                            <input type="datetime-local" name="publish_at" value="{{ old('publish_at', optional($post->publish_at)->format('Y-m-d\\TH:i')) }}" class="w-full border rounded p-2">
                            @error('publish_at') <p class="text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block mb-1 font-medium">Fecha fin (solo Novedad)</label>
                            <input type="datetime-local" name="expires_at" value="{{ old('expires_at', optional($post->expires_at)->format('Y-m-d\\TH:i')) }}" class="w-full border rounded p-2">
                    ...
                    <div class="flex gap-3">
                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded">Guardar cambios</button>
